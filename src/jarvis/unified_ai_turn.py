...
REPLY_ONLY_AFTER_MISROUTING_PROMPT = (
    "Пользователь задал информационный вопрос. Ответь кратко и по делу по-русски, обычным текстом. "
    "Без JSON, без перечня команд, не открывай сайты — только содержательный ответ."
)

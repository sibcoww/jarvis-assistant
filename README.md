...
- 🎯 **Dual Wake-word Detection** (Porcupine 11мс / Vosk 4-6с)
- ⚡ **Continuous Command Mode** (несколько команд без повтора wake word)
- 🖥️ **Modern GUI** (PySide6 с tray-режимом)
- 🔌 **Плагины** (расширяемая архитектура)
- ⏱️ **Detailed Timing Logs** (профилирование каждой стадии)

## 🌟 Ключевые особенности

### 1️⃣ Dual Wake-Word Engine
**Выбор между скоростью и стабильностью:**

**Porcupine (Picovoice):**
- ⚡ **11мс реакция** на wake-word "джарвис"
- 🎧 Фоновое прослушивание (не блокирует)
- 🎯 Высокая точность детекции
- 🔑 Требует `PICOVOICE_ACCESS_KEY`

**Vosk (текстовый):**
- 🔒 **Стабильный** (нет ложных срабатываний)
- 📦 **Полностью offline** (без внешних ключей)
- ⏱️ 4-6с задержка на детект wake-word
- ✅ Готов из коробки

**Переключение:** GUI → Настройки → Движок активации

### 2️⃣ ML-Based Natural Language Understanding
...
### 4️⃣ Timing Instrumentation & Performance
**Детальная телеметрия для оптимизации:**
- ⏱️ **Wake-word латентность:** от детекта до активации
- 📊 **Фраза → команда:** время распознавания
- 🚀 **End-to-end:** полный цикл от голоса до выполнения
- 📝 **Startup timing:** логируется в `~/.jarvis/startup_timing.log`

**Пример логов:**
```
⏱ От детекта wake-word до старта прослушивания: 11мс
⏱ Команда после wake-word: фраза за 2.39с
🧠 Интент: set_volume (confidence: 0.74)
⏱ Слушаю следующую команду... (10с)
```

### 5️⃣ Modern GUI Features
- 🪟 **Tray-резидентный режим:** сворачивание в трей вместо закрытия
- 🚀 **Background preload:** `--minimized` для автозапуска с Windows
- 📊 **Real-time progress:** привязка к загрузке модели
- ⚙️ **Settings panel:** выбор микрофона, таймауты, движок wake-word
- ☑️ **Windows autostart:** чекбокс для автозапуска через Startup

### 6️⃣ Поддержка словесных чисел
